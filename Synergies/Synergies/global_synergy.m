function [syn_struct,n_syn] = global_synergy (data)
% Function to process synergies from already concatenated EMG data. 
%INPUT: 
%     - data: matrix of n trials(or samples or whathever)* channels of EMG
%
...
% If name of the file is specified use it as an intro
if ischar(data)
    load(data);
end
...
% EMG _INIT_ NOW WORKS ON ITS OWN
% % if exist('data', 'var') 
...
for m = 1:nmus-1
   ...
   [syn_struct(m).W,syn_struct(m).H,syn_struct(m).D] = synergies(data,m,nmus,1);
   [syn_struct(m).mus_VAF, syn_struct(m).matx_VAF, syn_struct(m).ReconData,...
        syn_struct(m).trial_VAF,syn_struct(m).ReconR2]= synergy_vaf(data,syn_struct(m).W,syn_struct(m).H);     
end

n_syn = select_syn(syn_struct);
[Recon_W, W_Contr] = plot_synergies(syn_struct,n_syn,data);
